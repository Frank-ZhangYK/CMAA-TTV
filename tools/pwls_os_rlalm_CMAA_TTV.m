 function [xD] = pwls_os_rlalm_CMAA_TTV(xD, Ab, yiD, wiD, R, denomD, D_R, varargin)
...
%| 2020, modified by Yuanke Zhang

if nargin == 1 && streq(xD, 'test'), ir_pwls_os_rlalm_test, return, end
...
if (ndims(yiD) ~= 3) || (size(yiD,2) == 1 && nblock > 1)
	fail 'bad yi size'
end
if (ndims(wiD) ~= 3) || (size(wiD,2) == 1 && nblock > 1)
	fail 'bad wi size'
end
...
if ~arg.update_even_if_denom_0
	...
	denomD(denomD == 0) = inf; % trick: prevents pixels where denom=0 being updated
end
...
for ii=1:np
    ...
    resid = wiD(:,ia,ii) .* (li - yiD(:,ia,ii));
    zeta = [zeta; nblock * Ab{iblock}' * resid(:)];
end
...
for iter = 1:arg.niter
	...
	for iset = 1:nblock
		...
		num = rho(k) * (denomD(:) .* xD(:) - h) + (1-rho(k)) * g;
		den = rho(k) * denomD(:);

	    num = num + R.cgrad(xD);
        den = den + D_R;

		xD = xD - reshape(relax * num ./ den, size(xD));
		xD = max(xD, pixmin);
		xD = min(xD, pixmax);
        ...
        for ii=1:np
            ...
            li = reshape(li, nb, length(ia));
            resid = wiD(:,ia,ii) .* (li - yiD(:,ia,ii));
            zeta = [zeta; nblock * Ab{iblock}' * resid(:)];% A' * W * (y - A*x)
        end
        
		g = (rho(k) * (alpha * zeta + (1-alpha) * g) + g) / (rho(k)+1);
		h = alpha * (denomD(:) .* xD(:) - zeta) + (1-alpha) * h; 
    end
